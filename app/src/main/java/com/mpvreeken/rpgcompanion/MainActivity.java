...
public class MainActivity extends RPGCActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*
            News Alert
         */
        String news_url = getResources().getString(R.string.url_get_news);
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, final Response response) throws IOException {
                if (!response.isSuccessful()) {
                    ...
                    if (o.length()>0 && id>=0) {
                        final SharedPreferences prefs = MainActivity.this.getSharedPreferences("com.mpvreeken.rpgcompanion", Context.MODE_PRIVATE);
                        Integer last_news_id = prefs.getInt("news_id", 0);
                        if (last_news_id < id) {
                            ...
                            MainActivity.this.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    final ConstraintLayout l = findViewById(R.id.main_news_layout);
                                    l.setVisibility(View.VISIBLE);
                                    final TextView news_tv = findViewById(R.id.main_news_tv);
                                    final Button dismiss_btn = findViewById(R.id.main_news_dismiss_btn);

                                    news_tv.setText(output);
                                    dismiss_btn.setOnClickListener(new View.OnClickListener() {
                                        public void onClick(View v) {
                                            l.setVisibility(View.INVISIBLE);

                                            SharedPreferences.Editor editor = prefs.edit();
                                            editor.putInt("news_id", i);
                                            editor.apply();
                                        }
                                    });
                                }
